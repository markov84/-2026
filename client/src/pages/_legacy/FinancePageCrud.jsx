...
export default function FinancePageCrud() {
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      setData((current) => buildFinanceState((current?.entries || []).map((item) => (item._id === editingEntry._id ? response.data : item))));
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setData((current) => buildFinanceState((current?.entries || []).filter((item) => item._id !== deletingEntry._id)));
  ...
  return (
    <Stack spacing={3}>
      <PageHeader eyebrow="Финанси" title="Финансов дневник" subtitle="Приходи, разходи и банка с редакция и изтриване на всеки запис." />
      ...
      <DataSection title="Финансов дневник" subtitle="Редакция и триене на записи" actions={<Button variant="contained" onClick={() => setOpen(true)}>Добави запис</Button>}>
        <ResponsiveTable>
          <DataGrid
            autoHeight
            loading={loading}
